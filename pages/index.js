...
const Home = () => {
  ...
  const getGenreAndRatings = async (title, year, media_type) => {
    try {
      const options = {
        ...
        headers: {
          "X-RapidAPI-Key":
            "42cdf87f05msh759f3cb46d71080p139689jsnd123afa86728",
  ...
  const getStreamingServices = async (tmd_id, media_type) => {
    try {
      const url =
        media_type === "movie"
          ? `https://api.themoviedb.org/3/movie/${tmd_id}/watch/providers?api_key=cdb1e3404d89d5bb21c91edf2e2dce4c`
          : `https://api.themoviedb.org/3/tv/${tmd_id}/watch/providers?api_key=cdb1e3404d89d5bb21c91edf2e2dce4c`;
  ...
  const handleOnSearch = async (string, results) => {
    try {
      const res = await fetch(
        `https://api.themoviedb.org/3/search/multi?api_key=cdb1e3404d89d5bb21c91edf2e2dce4c&language=en-US&page=1&include_adult=false&query=${string}`
      );
      const data = await res.json();
